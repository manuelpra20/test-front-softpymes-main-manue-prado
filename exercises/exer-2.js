...
mapCompanies = (arrCompanies, hasCar) => {
    return arrCompanies.map(item => {

        const  users = item.users.filter(itemUser => {
            return itemUser.car === hasCar;
        });

        item.users = users;
        item.usersLength = users.length;
        return item;
    })
}
...
const newCompanies = mapCompanies(companies, false);
